from src.behavioral.observer import main, publisher, subscribers
...
class TestObserverPattern:
    def test_correct_value(self):
        ...
        assert expected_display == actual_display, f"Wrong value. Expected {expected_display} "
        f"but got {actual_display}."

        assert expected_log == actual_log, f"Wrong value. Expected {expected_log} "
        f"but got {actual_log}."

        assert expected_alert == actual_alert, f"Wrong value. Expected {expected_alert} "
